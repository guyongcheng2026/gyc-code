...
// 账号服务端点：优先自建（GYCCODE_ACCOUNT_URL，见 services/account），默认本地自建服务
const defaultServer = process.env.GYCCODE_ACCOUNT_URL ?? "http://localhost:8787"
...
function oauth(http: HttpClient.HttpClient) {
  return {
    integrationID: Integration.ID.make("gyccode"),
    method: {
      id: methodID,
      type: "oauth",
...
export const GyccodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "gyccode",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("GyccodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("gyccode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load GycCode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    connected = (yield* ctx.integration.connection.active("gyccode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("gyccode")
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("gyccode")),
