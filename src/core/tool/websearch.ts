...
export class ConfigService extends Context.Service<ConfigService, Config>()("@gyccode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.GYCCODE_WEBSEARCH_PROVIDER === "exa" || process.env.GYCCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.GYCCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("GYCCODE_EXPERIMENTAL") || truthy("GYCCODE_ENABLE_EXA") || truthy("GYCCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("GYCCODE_ENABLE_PARALLEL") || truthy("GYCCODE_EXPERIMENTAL_PARALLEL"),
