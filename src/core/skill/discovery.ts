...
export class Service extends Context.Service<Service, Interface>()("@gyccode/v2/SkillDiscovery") {}
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      pull: Effect.fn("SkillDiscovery.pull")(function* (url) {
        ...
        const sourceRoot = path.resolve(global.cache, "skills", Hash.sha256(base))
        return yield* Effect.forEach(
          data.skills.flatMap((skill) => {
            ...
            const versionFile = path.join(root, ".gyccode-version")
            ...
            Effect.gen(function* () {
              ...
              if (version === undefined || current === version) {
                ...
                yield* Effect.gen(function* () {
                  ...
                  if (!exists) return
                  yield* fs.writeFileString(path.join(staging, ".gyccode-version"), version)
