...
declare const GYCCODE_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof GYCCODE_LIBC === "undefined" ? undefined : GYCCODE_LIBC
...
export class Service extends Context.Service<Service, Interface>()("@gyccode/v2/FileWatcher") {}

const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.GYCCODE_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.GYCCODE_EXPERIMENTAL_FILEWATCHER)) {
