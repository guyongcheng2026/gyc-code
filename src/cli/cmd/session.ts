...
function pagerCmd(): string[] {
  ...
  if (Flag.GYCCODE_GIT_BASH_PATH) {
    const less = path.join(Flag.GYCCODE_GIT_BASH_PATH, "..", "..", "usr", "bin", "less.exe")
...
export const SessionDeleteCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.session.delete")(function* (args) {
    ...
    UI.println(UI.Style.TEXT_SUCCESS_BOLD + `已删除会话 ${args.sessionID}` + UI.Style.TEXT_NORMAL)
  }),
})
...
function formatSessionTable(sessions: Session.Info[]): string {
  ...
  const header = `会话 ID${" ".repeat(maxIdWidth - 10)}  标题${" ".repeat(maxTitleWidth - 5)}  更新时间`
  lines.push(header)
