import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@gyccode/protocol/plugin/tui"
...
import type { useGyccodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useGyccodeKeymap>
  ...
  return {
    ...
    session: {
      ...
      instructions(sessionID) {
        return sync.data.session_instructions[sessionID] ?? []
      },
      goal(sessionID) {
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getGyccodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getGyccodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
