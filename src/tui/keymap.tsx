...
const GYCCODE_MODE_KEY = "gyccode.mode"

export const GyccodeKeymapProvider = KeymapProvider
export const useGyccodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type GyccodeModeStack = ReturnType<typeof createGyccodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, GyccodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createGyccodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(GYCCODE_MODE_KEY, GYCCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(GYCCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(GYCCODE_MODE_KEY, stack.at(-1)?.mode ?? GYCCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? GYCCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(GYCCODE_MODE_KEY, undefined)
...
export function useGyccodeModeStack() {
  return getGyccodeModeStack(useGyccodeKeymap())
}

export function getGyccodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Gyccode mode stack is not registered for this keymap")
  return value
}
...
export function registerGyccodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createGyccodeModeStack(keymap)
