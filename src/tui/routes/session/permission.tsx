...
import type { PermissionRequest } from "@gyccode/protocol/v2"
          ...
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={"这将允许 " + props.request.permission + "，直到 GycCode 重启。"} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>这将允许以下模式，直到 GycCode 重启</text>
                  <box>
        ...
        {(() => {
          const info = () => {
            ...
            if (permission === "webfetch") {
              ...
              return {
                ...
                body: (
                  <Show when={url}>
                    <box paddingLeft={1}>
                      <text fg={theme.textMuted}>{"网址： " + url}</text>
                    </box>
                  </Show>
                ),
              }
            ...
            if (permission === "websearch") {
              ...
              return {
                icon: "◈",
                title: `搜索 ${webSearchProviderLabel(data.provider)} "${query}"`,
                body: (
                  <Show when={query}>
                    <box paddingLeft={1}>
            ...
            if (permission === "doom_loop") {
              return {
                ...
                body: (
                  <box paddingLeft={1}>
                    <text fg={theme.textMuted}>即使反复失败，也保持会话继续运行。</text>
                  </box>
                ),
              }
            }
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>告诉 GycCode 应当如何调整</text>
        </box>
      </box>
      <box
