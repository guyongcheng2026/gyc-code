...
import { SkillPlugin } from "@gyccode/core/plugin/skill"
import { Permission } from "@/permission"
import { FSUtil } from "@gyccode/core/fs-util"
import { Config } from "@/config/config"
import { FrontmatterError } from "@gyccode/core/v1/config/error"
...
// Built-in skill that ships with gyccode. The model's intuition for what an
// gyccode.json should look like is often wrong, and gyccode hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch gyccode's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_GYCCODE_SKILL_NAME = "customize-gyccode"
const CUSTOMIZE_GYCCODE_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating gyccode's own configuration: gyccode.json, gyccode.jsonc, files under .gyccode/, or files under ~/.config/gyccode/. Also use when creating or fixing gyccode agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring gyccode itself."
const CUSTOMIZE_GYCCODE_SKILL_BODY = SkillPlugin.CustomizeGyccodeContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, GYCCODE_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@gyccode/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_GYCCODE_SKILL_NAME] = {
          name: CUSTOMIZE_GYCCODE_SKILL_NAME,
          description: CUSTOMIZE_GYCCODE_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_GYCCODE_SKILL_BODY,
        }
