...
export function maybeParseApplyPatch(
  ...
  const patchArg = argv[1]
  if (argv.length === 2 && command !== undefined && patchArg !== undefined && APPLY_PATCH_COMMANDS.includes(command)) {
    try {
      const { hunks } = parsePatch(patchArg)
      return {
...
function applyReplacements(lines: string[], replacements: Array<[number, number, string[]]>): string[] {
  ...
  for (let i = replacements.length - 1; i >= 0; i--) {
    ...
    for (let j = 0; j < newSegment.length; j++) {
      const segmentLine = newSegment[j]
      if (segmentLine === undefined) continue
      result.splice(startIdx + j, 0, segmentLine)
    }
  }
...
export const maybeParseApplyPatchVerified = Effect.fn("Patch.maybeParseApplyPatchVerified")(function* (
  ...
  const implicitArg = argv[0]
  if (argv.length === 1 && implicitArg !== undefined) {
    try {
      parsePatch(implicitArg)
      return {
