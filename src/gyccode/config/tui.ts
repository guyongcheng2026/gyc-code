...
import { isRecord } from "@gyccode/tui/util/record"
...
import { TuiKeybind } from "@gyccode/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@gyccode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old gyccode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.gyccode`
  // folders between cwd and home, and GYCCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.GYCCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit GYCCODE_TUI_CONFIG override, if set.
  if (Flag.GYCCODE_TUI_CONFIG) {
    const configFile = Flag.GYCCODE_TUI_CONFIG
  ...
  // 4. `.gyccode` directories (and GYCCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".gyccode") || dir === Flag.GYCCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".gyccode") && dir !== Flag.GYCCODE_CONFIG_DIR) continue
