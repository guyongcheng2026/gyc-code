...
const TUI_SCHEMA_URL = "https://gyccode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from gyccode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const gyccode = await gyccodeFiles(input)
  for (const file of gyccode) {
...
async function gyccodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "gyccode"),
    ...(await Filesystem.findUp(["gyccode.json", "gyccode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "gyccode"))
  }
  if (Flag.GYCCODE_CONFIG) files.push(Flag.GYCCODE_CONFIG)
