...
import type { ConsoleState } from "@gyccode/core/v1/config/console-state"
...
import { RemoteAuthError } from "@gyccode/core/v1/config/error"
import { ConfigPermissionV1 } from "@gyccode/core/v1/config/permission"
import { ConfigPluginV1 } from "@gyccode/core/v1/config/plugin"
...
export class Service extends Context.Service<Service, Interface>()("@gyccode/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["gyccode.jsonc", "gyccode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://gyccode.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.GYCCODE_CONFIG && !Flag.GYCCODE_CONFIG_DIR && !Flag.GYCCODE_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://gyccode.ai/config.json" }, null, 2))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://gyccode.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "GYCCODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/gyccode`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://gyccode.ai/config.json"
        ...
        if (Flag.GYCCODE_CONFIG) {
          yield* merge(Flag.GYCCODE_CONFIG, yield* loadFile(Flag.GYCCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.GYCCODE_CONFIG })
        }

        if (!Flag.GYCCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("gyccode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.GYCCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from GYCCODE_CONFIG_DIR", { path: Flag.GYCCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".gyccode") || dir === Flag.GYCCODE_CONFIG_DIR) {
            for (const file of ["gyccode.json", "gyccode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.gyccode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.GYCCODE_CONFIG_CONTENT) {
          const source = "GYCCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.GYCCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from GYCCODE_CONFIG_CONTENT")
        }
        ...
        if (Flag.GYCCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.GYCCODE_DISABLE_PRUNE) {
