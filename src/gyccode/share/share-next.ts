...
import type * as SDK from "@gyccode/protocol/v2"
...
import { SessionShareTable } from "@gyccode/core/share/sql"
import { ProviderV2 } from "@gyccode/core/provider"
import { ModelV2 } from "@gyccode/core/model"
import { EventV2 } from "@gyccode/core/event"

const disabled = process.env["GYCCODE_DISABLE_SHARE"] === "true" || process.env["GYCCODE_DISABLE_SHARE"] === "1"
...
export class Service extends Context.Service<Service, Interface>()("@gyccode/ShareNext") {}
  ...
  Effect.gen(function* () {
    ...
    const request = Effect.fn("ShareNext.request")(function* () {
      ...
      if (Option.isNone(active) || !active.value.active_org_id) {
        const baseUrl = (yield* cfg.get()).enterprise?.url ?? "http://localhost:8788"
