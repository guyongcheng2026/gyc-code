...
export const DEPRECATED_PLUGIN_PACKAGES = ["gyccode-openai-codex-auth", "gyccode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, gyccodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(gyccodeVersion) || semver.major(gyccodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.gyccode
  if (typeof range !== "string") return
  if (!semver.satisfies(gyccodeVersion, range)) {
    throw new Error(`Plugin requires gyccode ${range} but running ${gyccodeVersion}`)
  }
}
