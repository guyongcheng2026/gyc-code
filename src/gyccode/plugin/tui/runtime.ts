...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@gyccode/protocol/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@gyccode/core/effect/app-node-builder"
import { errorData, errorMessage } from "@gyccode/tui/util/error"
...
import { hasTheme, upsertTheme } from "@gyccode/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@gyccode/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@gyccode/core/v1/config/plugin"
import { createCommandShim } from "@gyccode/tui/plugin/command-shim"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".gyccode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".gyccode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".gyccode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.GYCCODE_PURE ? [] : pluginOrigins
    if (Flag.GYCCODE_PURE && pluginOrigins.length) {
    }
