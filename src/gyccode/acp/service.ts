...
export const AuthMethodID = "gyccode-login"
...
export class Service extends Context.Service<Service, Interface>()("@gyccode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `gyccode auth login` in the terminal",
      name: "Login with gyccode",
      id: AuthMethodID,
    }
...
async function loadDirectorySnapshot(sdk: GyccodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, gyccode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const gyccodeProvider = providers[ProviderV2.ID.make("gyccode")]
  const gyccodeModel = gyccodeProvider ? Provider.sort(Object.values(gyccodeProvider.models))[0] : undefined
  if (gyccodeProvider && gyccodeModel) return { providerID: gyccodeProvider.id, modelID: gyccodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "GycCode service failure", service })
}
