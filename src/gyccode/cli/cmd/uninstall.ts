...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall gyc and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall GycCode")
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  for (const dir of targets.directories) {
    ...
    if (err) {
      spinner.stop(`Failed to remove ${dir.label}`)
  ...
  if (targets.shellConfig) {
    ...
    if (err) {
      spinner.stop("Failed to clean shell config")
  ...
  UI.empty()
  prompts.log.success("Thank you for using GycCode!")
}
