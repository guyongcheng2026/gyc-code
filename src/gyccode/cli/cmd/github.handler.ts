...
const AGENT_USERNAME = "gyccode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/gyccode.yml"
...
jobs:
  gyccode:
    if: |
      ...
      contains(github.event.comment.body, ' /gyccode') ||
      startsWith(github.event.comment.body, '/gyccode')
    ...
    steps:
      ...
      - name: Run gyccode
        uses: anomalyco/gyccode/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/gyccode,/oc")
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to gyccode...")
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `gyccode/${type}-${hex}-${timestamp}`
      }
      return `gyccode/${type}${issueId}-${timestamp}`
    }
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the gyccode infrastructure after your response",
