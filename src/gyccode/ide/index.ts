...
import { IdeEvent } from "@gyccode/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["GYCCODE_CALLER"] === "vscode" || process.env["GYCCODE_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.gyccode"], {
    nothrow: true,
  })
