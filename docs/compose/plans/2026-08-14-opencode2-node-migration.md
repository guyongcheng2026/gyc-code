...
- [x] **Step 1: 用 bun build 试产 Node 目标**
...
- [x] **Step 2: 验证产物能否被 Node 解析（仅解析，不完整启动）**
...
- [x] **Step 1: 安装 string-width 依赖**

```bash
bun add string-width
```

- [x] **Step 2: 写失败测试**（对比 Bun.stringWidth，保证替换语义一致）
...
- [x] **Step 3: 实现（替换 Bun.file/Bun.write）**
...
- [x] **Step 1: 写失败测试**（spawn 子进程注入 stdin 验证）
...
- [x] **Step 1: 修改 Node 分支为直接 import dist**
...
- [x] **Step 1: 定义接口（对齐 terminal-win32 现有 8 个符号）**
...
- [x] **Step 2: bun 实现（bun:ffi，逻辑照搬 terminal-win32 现有 kernel() 函数）**
...
- [x] **Step 3: node 实现（koffi，已验证 API）**
...
- [x] **Step 5: terminal-win32.ts 改用适配层**
...
- **状态**：54 个 checkbox 全部标记完成（[x]）。升级内核（阶段 1：Bun → Node 运行时）已全部落地，无未完成项。
- **验证方式**（2026-08-15 复跑确认）：
  - `bun tsc --noEmit` → exit 0，无类型错误。
  - `bun test`（全量）→ 463 pass / 0 fail（71 文件，62s）。
  - `GYC_RUNTIME=node bun build.mjs` → dist/（node 目标）与 dist-bun/（bun 目标）双产物生成，RUNTIME 标记正确。
  - `node dist/index.js --version` → 0.0.1，exit 0（Node 直跑，无 Bun spawn）。
  - 生产代码 Bun 残留：仅 plugin/index.ts:164 的 `Bun.$` 运行时探测（typeof 守卫，Node 下安全），dist 产物无 `bun:` 硬依赖。
- **与计划的两处实现偏差（均为合理修正，记录在案）**：
  1. Task 1B：计划用 `SetConsoleInputCP`/`GetConsoleInputCP`（不存在于 kernel32），实际改用 `SetConsoleCP`/`GetConsoleCP`（见 aecf004）。
  2. Task 1A/1B 条件导出：计划写在 `src/core/package.json`，实际写在 `src/tui/package.json`（因调用方 editor-zed.ts / terminal-win32.ts 位于 src/tui/，需在就近 package.json 声明 #zed-sqlite / #win32-kernel 才能解析）。
- **阶段 2 遗留（本计划范围外，文档已注明）**：测试运行器迁移（bun test→node --test/vitest）、默认运行时切 Node 的安装分发、常驻服务模式评估、`@tsconfig/bun`→`@tsconfig/node22`。
