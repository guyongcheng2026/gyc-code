...
> **核对状态（2026-09-24）**：P1 共 26 条经文件级核对——20 条文件仍存在（保留跟踪；已知已修例：`prompt.ts:80-86` 冷却清理已落地 `cleanupExpiredCooldowns`），6 条引用路径已变迁/消亡（`src/gyccode/cli/*` 迁至 `src/cli/*`、`memory/index.ts`/`skills/index.ts` 重复导出已随重构消失）。本报告行号系审查时点快照，**行级状态已漂移**；逐条重开须以现行代码重新审计，勿按原行号直接修改。现状基线：tsc 0 错误、全量 1030 pass / 0 fail。
