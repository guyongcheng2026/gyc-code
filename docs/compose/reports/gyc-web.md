...
浏览器交互：左栏会话列表 + 文件树（git 角标）→ 主区聊天/改动/文件（monaco）→ 底部终端；工具请求弹出审批卡片。
...
- **webapp 单测**：vitest → 8 文件 14 用例全过（reducer/hooks/组件/审批/文件树）。
...
### 已知限制（预存 server 侧，非 webapp 引入）

- **PTY server 500**：`pty.create` 在 Node 运行时返回 500，且 core `Pty.Service` 在 Bun 直测也报 `self.build` 未定义（`bun-pty` 内部引用 `Bun.build`）。这是**既有 server 基础设施 bug**，阻塞终端的端到端验证；webapp 终端组件按协议正确实现并处理错误。修复列为独立任务（排查 `bun-pty`/`@lydell/node-pty` 集成）。
