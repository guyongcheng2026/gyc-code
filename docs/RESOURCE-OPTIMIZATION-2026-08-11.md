...
## 六、实际执行结果（用户确认后）
### 1. 多实例收敛（已完成）
- 进程树确认当前实例 = bun PID 6544（本会话，shell 父链直指），保留
- 终止陈旧实例 PID 6676（08-09 启动、空闲 2 天、CPU 3s 内仅 0.02s）及其启动器 596（随子进程退出自动结束）
- 结果：3 个 bun 实例 → 1 个（当前 6544），合计 WS 从 ~1.5GB 降至 ~907MB

### 2. event 保留期实际裁剪
- `--prune-events=12`：删 790 行 / 1.5MB（空闲 22.5h/23.4h 会话）
- 用户改选 `--prune-events=8`：删 3,959 行 / 5.4MB（ses_014c26f7 9MB 会话）
- 注：12h 不会裁剪空闲仅 9h 的 9MB 会话（<12h），故改 8h；8h 下该会话 6,498→2,539 条（=distinct part 数，每 part 保留最新一条），9MB→3.59MB
- 全程 integrity ok；活跃会话（ses_01046ff1，5.8MB，实时写入）未触碰
- 建议定期执行：`bun scripts/maintain-data.mjs --prune-events=8 --clean --checkpoint`
