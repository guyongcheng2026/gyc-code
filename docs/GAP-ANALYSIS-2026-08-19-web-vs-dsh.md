...
## ???????? DSH sidebar ??????
- `client/useSessions.ts`?`SessionItem` ?? `parentID?`?v1 session list ??? parentID?93 ?? 24 ??????
- `app/SessionList.tsx`??????????? parentID ???????? + ????`sidebar-item-child`??parent ????????
- `index.css`??? `sidebar-item-child` ????/?????

## ??????????? DSH Jobs ???
| ? | DSH ?? | ?? |
|---|---|---|
| ???? | ??????/???? | ???? `GET /experimental/jobs`?httpapi experimental group + handler ? `background.list()`??? JobInfo?id/type/title/status/started_at/completed_at/output/error/metadata???? `client/useJobs.ts` ?? 3s + `session.busy/idle/updated` ?????? |
| ???? | running/completed/error/cancelled | `app/JobsPanel.tsx` ??????????????/???/????+ ?? + ??/?????????????? |
| ??/?? | ???????? | ??????????????? id = job id??????????? v2 `session.abort` ??? |

## ????
- `bun run build` ???vite web UI + node dist?
- webapp vitest 8 ?? 14 ????
- ?? 4096 ????? `GET /experimental/jobs` ?? `[]`????????? 200

## ?????????????
- ???????/??????? message-feedback ??????
- ???? + ?????/??/??/????P2?
- ????????OS ???web ??? input webkitdirectory ???
- ??????????????????????
- TTFT/???????????? timing ???
- ??? GoalBar?? goal ?????
